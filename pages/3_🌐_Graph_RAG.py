# GRAPH_RAG.py - Single page version for app.py integration
    ...
    def _groq_analysis(self, question, relationship_data, confidence):
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are a professional document analyst. Use the relationship data to provide insightful answers."},
...
class DocumentProcessor:
    ...
    def _extract_pdf_text(self, pdf_path):
        ...
        return self._extract_text_direct(pdf_path) if pdf_type == "text_based" else self._extract_text_ocr(pdf_path)
...
- **Document**: {st.session_state.graph_rag_document}
- **📄 Pages Processed**: {stats.get('pages', 0)}
- **📝 Paragraphs Extracted**: {stats.get('paragraphs', 0)}
- **🔗 Entities Identified**: {stats.get('entities', 0)}
- **🕸️ Total Nodes**: {stats.get('pages', 0) + stats.get('paragraphs', 0) + stats.get('entities', 0)}

💡 Ask about relationships between concepts in your document!
                    """
                })
                st.rerun()
